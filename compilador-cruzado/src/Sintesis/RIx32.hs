...
-- Transforma un Operando de la representación de 3 direcciones a un Oper para la arquitectura.
...
traduce (InsUnaria a op b) i = case op of
			       '-' -> case opA of
			       	      (Registro s) -> ([Move (Cons1 0) opA, Sub opB opA], i+1)
			     	      (Var1 s) -> ([Move (Cons1 0) (Registro re), Sub opB (Registro re), Move (Registro re) opA], i+1)
			       where opA = operador32 a
			             opB = operador32 b
			             re = obtenerR i
